...
create policy "Follow peers in same bucket"
  ...
  with check (
    ...
    and exists (
      select 1
      from profiles me
      join profiles them on them.id = community_follows.followee_id
      where me.id = auth.uid()
        and me.primary_goal = them.primary_goal
        and me.experience_level = them.experience_level
        and them.gamification_opt_in = true
    )
  );
