...
export function CommunityHero({ gamification }: CommunityHeroProps) {
  ...
  if (!gamification.unlocked) {
    return (
      <section className="relative overflow-hidden rounded-3xl border border-forge-ember/25 bg-gradient-to-br from-forge-surface-raised via-forge-surface to-forge-ember/10 p-5 sm:p-6">
        ...
        <h2 className="mt-2 font-display text-2xl font-bold text-forge-text">
  ...
  if (!gamification.bucketLabel) {
    return (
      <section className="rounded-3xl border border-[var(--border)] bg-forge-surface-raised p-5 sm:p-6">
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-forge-gold/20 bg-gradient-to-br from-forge-surface-raised via-[#1a1410] to-forge-ember/15 p-5 sm:p-6">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
        ...
        {gamification.optedIn && gamification.userScore != null && (
          <div className="text-right">
            ...
            <p className="font-display text-3xl font-bold tabular-nums text-forge-gold">
      ...
      <div className="relative mt-6 flex items-end gap-4">
        <div>
          ...
          {hasRank ? (
            <p className="font-display text-6xl font-bold leading-none tabular-nums text-forge-text sm:text-7xl">
              #{gamification.userRank}
            </p>
          ) : (
            <p className="mt-1 font-display text-3xl font-bold text-forge-muted">
              —
            </p>
          )}
          {!gamification.optedIn && (
      ...
      {chasing && gamification.optedIn && (
        <div className="relative mt-5 rounded-2xl border border-forge-gold/35 bg-forge-surface/80 px-4 py-3 backdrop-blur-sm">
      ...
      {!gamification.optedIn && (
        <div className="relative mt-5 rounded-2xl border border-forge-ember/30 bg-forge-ember/10 px-4 py-4">
          ...
          <button
            ...
            disabled={saving}
            className="mt-3 w-full rounded-xl bg-forge-ember px-4 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-60 sm:w-auto"
          >
      ...
      {league && league.badges.length > 0 && gamification.optedIn && (
