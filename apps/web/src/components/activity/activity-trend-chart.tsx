...
export function ActivityTrendChart({ series }: ActivityTrendChartProps) {
  ...
  const hasData = series.some((point) => point.steps != null);
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(value, name, item) => {
                const payload = item.payload as { activeMinutes?: number };
                if (name === "steps") {
                  return [
                    `${Number(value).toLocaleString()} steps · ${payload.activeMinutes ?? 0} active min`,
                    "Daily activity",
                  ];
                }
