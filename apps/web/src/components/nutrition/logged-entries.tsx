...
interface LoggedEntriesProps {
  ...
  /** Omit outer section shell when nested inside a parent card */
  embedded?: boolean;
}
      ...
      {entries.length === 0 ? (
        ...
        <div className="mt-4 flex flex-col gap-3">
          {entries.map((entry) => (
            <article
              key={entry.id}
              className="flex items-start justify-between gap-3 rounded-xl border border-[var(--border)] bg-forge-surface p-4"
            >
