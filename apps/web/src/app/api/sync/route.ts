...
const syncSchema = z.object({
  sessions: z.array(
    z.object({
      ...
      sessionSource: z.enum(["program", "custom", "imported"]).optional(),
  ...
  sets: z.array(
    z.object({
      ...
      plannedExerciseId: z.string().min(1).optional(),
      substitutionReason: z.enum(["equipment_busy", "user_choice"]).optional(),
      completed: z.boolean(),
      completedAt: z.string().optional(),
      updatedAt: z.string().min(1),
    })
...
export async function POST(request: Request) {
      ...
      if (isTerminal || incomingUpdated >= existingUpdated) {
        const { error } = await supabase
          ...
          .update({
            ...
            session_source: session.sessionSource ?? "program",
  ...
  for (const set of body.sets) {
    ...
    const row = {
      ...
      planned_exercise_id: set.plannedExerciseId ?? null,
      substitution_reason: set.substitutionReason ?? null,
