...
9. **Custom day assignments (Phase 11, Pro)** — assign templates to calendar dates; Replace vs Keep both when a day already has a program/custom; hub merges assigned cards into “This week”; **Clear week & use custom** hides the active plan week via `user_workout_week_program_clears` and bulk-replaces that week’s custom assignments (`POST /api/workout-week-clear`)
...
5. `isExerciseAvailable` treats load modalities as **OR** and supports (`bench`, `squat_rack`) as **AND**; bodyweight movements are always eligible (see [ADR 004](./ADRs/004-exercise-equipment-matching.md))
