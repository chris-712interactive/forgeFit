...
10. **Clear week (Phase 11, Pro)** — `user_program_week_clears` hides all program cards for the active plan week so members can fill it with customs; Restore brings the plan back
...
| `/api/workout-week-clears` | Clear / restore program week for custom replacement (Pro) | 11 |
